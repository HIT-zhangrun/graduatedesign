...
num = 0;
%% 文件路径名称
%fname='../../../data/zr_4_24_1443_data/3/1_641.bin';
%fname='../../../data/zr_4_24_1443_data/3/1_639.bin';
...
function [range, f] = fft_process(data_origin)
    ...
    data_origin  = hanning(ADC_N)' .* data_origin(1 : ADC_N);
...
function [range, f] = phase_diff(data_origin)
    ...
    n_1 = round((x-1) * fs * T / (2 * M));
    fprintf("%f,%f,%f\n",(x_1-1) * fs * T / (2 * M),n_1 * 2 / T, delta_phase / (pi * T));
