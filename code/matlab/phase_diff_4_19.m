...
close all;
%% 参数全局化
global ADC_N fs N N_phase M B f_begin c f_slope T RX chirp num;
%% 参数设置
ADC_N   = 600;                 %采样点数
fs      = 15e6;                %采样频率
N       = 4096*4;                %FFT点数
...
function [range, f] = phase_diff(data_origin)
    %% 参数全局化
    global ADC_N fs N_phase M B f_begin c f_slope T RX chirp num;
    ...
    n_1 = round(x_1 * ADC_N / (2 * M));
    frequent_B = n_1 * 2 * fs / ADC_N + fs * delta_phase / (pi * ADC_N);
    %fprintf("相位差测频%f\n", frequent_B);
